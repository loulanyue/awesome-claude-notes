 ...
 *   3. Plugin cache auto-detection — scans ~/.claude/plugins/cache/ for
 *      both the current and legacy plugin cache names
...
function resolveEccRoot(options = {}) {
  ...
  for (const pluginName of PLUGIN_CACHE_CANDIDATES) {
    try {
      const cacheBase = path.join(claudeDir, 'plugins', 'cache', pluginName);
      ...
      // Candidate cache doesn't exist or isn't readable — try the next one
    }
  }
 ...
 */
const INLINE_RESOLVE = `(()=>{var e=process.env.CLAUDE_PLUGIN_ROOT;if(e&&e.trim())return e.trim();var p=require('path'),f=require('fs'),h=require('os').homedir(),d=p.join(h,'.claude'),q=p.join('scripts','lib','utils.js'),n=['awesome-claude-notes','everything-claude-code'];if(f.existsSync(p.join(d,q)))return d;try{for(var i of n){var b=p.join(d,'plugins','cache',i);for(var o of f.readdirSync(b))for(var v of f.readdirSync(p.join(b,o))){var c=p.join(b,o,v);if(f.existsSync(p.join(c,q)))return c}}}catch(x){}return d})()`;
