...
# 配置 awesome claude notes (ECC)

一个交互式、分步安装向导，用于 awesome claude notes 项目。使用 `AskUserQuestion` 引导用户选择性安装技能和规则，然后验证正确性并提供优化。
...
1. **通过插件**: `/plugin install awesome-claude-notes` — 插件会自动加载此技能
...
将 `ECC_ROOT=/tmp/awesome-claude-notes` 设置为所有后续复制操作的源。
