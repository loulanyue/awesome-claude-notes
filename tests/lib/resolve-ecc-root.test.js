...
function setupPluginCache(homeDir, orgName, version) {
  const cacheDir = path.join(
    ...
    'awesome-claude-notes', orgName, version
  );
...
function setupLegacyPluginCache(homeDir, orgName, version) {
  const cacheDir = path.join(
    homeDir, '.claude', 'plugins', 'cache',
...
function runTests() {
  ...
  if (test('discovers plugin root from cache directory', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupPluginCache(homeDir, 'awesome-claude-notes', '1.8.0');
      const result = resolveEccRoot({ envRoot: '', homeDir });
  ...
  if (test('discovers legacy plugin root from cache directory', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginCache(homeDir, 'everything-claude-code', '1.8.0');
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('prefers standard install over plugin cache', () => {
    ...
    try {
      ...
      setupPluginCache(homeDir, 'awesome-claude-notes', '1.8.0');
      const result = resolveEccRoot({ envRoot: '', homeDir });
  ...
  if (test('handles multiple versions in plugin cache', () => {
    const homeDir = createTempDir();
    try {
      setupPluginCache(homeDir, 'awesome-claude-notes', '1.7.0');
      const expected = setupPluginCache(homeDir, 'awesome-claude-notes', '1.8.0');
      const result = resolveEccRoot({ envRoot: '', homeDir });
      // Should find one of them (either is valid)
      assert.ok(
        result === expected ||
        result === path.join(homeDir, '.claude', 'plugins', 'cache', 'awesome-claude-notes', 'awesome-claude-notes', '1.7.0'),
  ...
  if (test('INLINE_RESOLVE discovers plugin cache when env var is unset', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupPluginCache(homeDir, 'awesome-claude-notes', '1.9.0');
      const { execFileSync } = require('child_process');
      const result = execFileSync('node', [
        '-e', `console.log(${INLINE_RESOLVE})`,
      ], {
